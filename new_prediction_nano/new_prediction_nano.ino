...
void loop()
{
    ...
    float resultsPrediction[2];
    for (size_t ix = 0; ix < EI_CLASSIFIER_LABEL_COUNT; ix++) {
        ei_printf("    %s: %.5f\n", result.classification[ix].label, result.classification[ix].value);
        resultsPrediction[ix]= result.classification[ix].value;
        ei_printf("    RESULTADO EN ARRAY: %.3f\n", resultsPrediction[ix]);
    }
    if(resultsPrediction[0]>resultsPrediction[1]){
      result_prediction=FALL;
    }
    else{
      result_prediction=NORMAL;
    }
